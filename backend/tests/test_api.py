...
def test_list_cases_success(client, auth_headers, respx_mock):
    respx_mock.get(f"{INTAKE_URL}/cases", params={"lawyer_id": 1}).mock(
        return_value=Response(200, json=[{"id": CASE_ID, "name": "Test Case"}])
    )
...
def test_get_case_details_success(client, auth_headers, respx_mock):
    ...
    respx_mock.get(f"{LAWYER_URL}/messages/{CASE_ID}").mock(
        return_value=Response(200, json=[{"sender_role": "ai_lawyer", "content": "How can I help?"}])
    )
...
def test_assist_case_success(client, auth_headers, respx_mock):
    ...
    respx_mock.post(f"{LAWYER_URL}/analyze").mock(
        return_value=Response(200, content=b'data: {"token": "Analysis"}\n\ndata: {"done": true}\n\n')
    )
