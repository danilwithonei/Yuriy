...
def _expired_token():
    payload = {"sub": "999999", "exp": datetime.now(UTC).replace(tzinfo=None) - timedelta(hours=1)}
...
class TestRegister:
    ...
    def test_register_duplicate_email(self, client):
        client.post("/auth/register", json={"email": "dup@test.com", "name": "First", "password": "pass1234"})
        resp = client.post("/auth/register", json={"email": "dup@test.com", "name": "Second", "password": "pass1234"})
        assert resp.status_code == 409
        assert "already registered" in resp.json()["detail"].lower()

    def test_register_invalid_email(self, client):
        resp = client.post("/auth/register", json={"email": "notanemail", "name": "Bad Email", "password": "pass1234"})
...
class TestLogin:
    def test_login_wrong_password(self, client):
        resp = client.post("/auth/login", json={"email": "lawyer_a@test.com", "password": "wrongpassword"})
